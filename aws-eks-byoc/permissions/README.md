...
- Capture also picks up grants with no consumer in the Terraform. IAM user/group **writes**, `ec2:AuthorizeClientVpnIngress` and ECR/ELB actions were present in all four policy files without any resource of that type being declared by `infra/aws-provision-karpenter` or `infra/aws-provision`; they have since been removed. Before adding an action, confirm something actually calls it.
- The converse also happens: a required action can have no visible consumer. Nothing in the Terraform launches an instance, but `eks:CreateNodegroup` with a launch template performs an `ec2:RunInstances` authorization check *as the caller* and fails with `You are not authorized to launch instances with this launch template` without it. The `NodeGroupLaunchTemplateAuthorization` statement grants it, gated on `ec2:LaunchTemplate` so the role cannot launch instances ad hoc. Do not gate it on `ec2:IsLaunchTemplateResource`: EKS supplies the AMI and subnets outside the template, so the `image`/`network-interface` resources evaluate that key as `false` and the check is denied. Verify with `aws ec2 run-instances --dry-run --launch-template LaunchTemplateId=<id> --image-id <eks-ami> --subnet-id <subnet> --instance-type <type>` as the role.
- The capture was taken with `enable_flow_log = false`, so opt-in paths are not covered. VPC Flow Logs (`ec2:CreateFlowLogs`/`DescribeFlowLogs`/`DeleteFlowLogs`) are included in the non-BYO policies because `infra/aws-provision-karpenter` creates them when `enable_flow_log` is set; the CloudWatch log group, IAM role, and IAM policy the module also creates (`vpc-flow-logs.tf` in `terraform-aws-modules/vpc`) only use `logs:`/`iam:` actions already granted for the EKS cluster log group and IRSA roles (`CreateLogGroup`, `PutRetentionPolicy`, `CreateRole`, `CreatePolicy`, `AttachRolePolicy`, `PassRole`, tagging/read/delete counterparts).
