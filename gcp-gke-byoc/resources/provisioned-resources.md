...
| `cloudkms.googleapis.com` | GKE secrets encryption |
...
| Resource | Count | Description |
|----------|-------|-------------|
| GKE cluster (`ryvn-gke-<env>`) | 1 | Regional, private nodes, private control plane endpoint, IAM-protected DNS access, VPC-native, Workload Identity enabled, Dataplane V2 (optional), secrets encrypted with the Cloud KMS key below, HTTP load balancing add-on off, Cloud Logging and Monitoring off |
...
| Cloud KMS key (`ryvn-gke-<env>-secrets`) | 1 | Symmetric, rotation enabled, with `roles/cloudkms.cryptoKeyEncrypterDecrypter` granted to the GKE service agent. See [deprovisioning](../permissions/README.md#deprovisioning) for cleanup behavior |
...
| Project binding for the provisioner | 1 | `roles/container.developer` granted to the provisioning service account |
